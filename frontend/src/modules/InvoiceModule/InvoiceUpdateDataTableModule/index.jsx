...
export default function InvoiceUpdateDataTableModule({ config }) {
    const dispatch = useDispatch();

    const handleApprove = async (record) => {
        try {
            const response = await request.post({ entity: 'invoice/approve', id: record._id });
            if (response.success) {
                notification.success({ message: 'Request Approved Successfully' });
                dispatch(erp.list({ entity: 'invoiceUpdate' }));
            } else {
